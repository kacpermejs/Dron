...
void Graniastoslup6::UstawWierzcholki()
    {
        ...
        for(unsigned int i=0; i < 12; ++i)
        {
            if(i<6)
            {
                Przesuniecie=MacierzOb(kat, 'z')*Wektor3D{m_promien,0,-1*(m_wysokosc)/2};
                kat+=60;
            }
            else
            {
                if(i==6)
                    kat=0;
                Przesuniecie=MacierzOb(kat, 'z')*Wektor3D{m_promien,0,(m_wysokosc)/2};
                kat+=60;
            }
...
void Graniastoslup6::przesun(Wektor3D V)
{
    m_WekPol+=V;
}